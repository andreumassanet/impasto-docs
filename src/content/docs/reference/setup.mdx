...
| `system` | Copies `system/` into `/` with sudo — the login screen, the lock screen's PAM service for face unlock and the rest — creates `/var/lib/impasto/faces`, and enables sddm from the next boot where no other display manager is enabled. Loads `i2c-dev` for the brightness of external monitors, so the session after the install has it without a reboot. | After a change to the login screen or the other system files. |
